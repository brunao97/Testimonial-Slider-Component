...
const CARD_HEIGHT = 208; // w-52 h-52
const GAP = 24; 
...
const App: React.FC = () => {
  ...
  // Calculate the center index to determine offset for the main column
  const listCenterIndex = Math.floor(TESTIMONIALS.length / 2);

  // Motion variants for the text
  const textVariants = {
  ...
  // Helper component for the architectural placeholder cards
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-[#f9fafb] selection:bg-gray-100">
      {/* Fixed Dimensions Container: 1100px width, 520px height */}
      <div className="w-[1100px] h-[520px] bg-white rounded-[48px] shadow-[0_40px_100px_-30px_rgba(0,0,0,0.08)] overflow-hidden flex border border-gray-100/80 relative">
        ...
        {/* Left Side: The "Frame" (42% width fixed) */}
        <div className="w-[42%] bg-[#fcfcfd] flex items-center justify-center relative overflow-hidden border-r border-gray-50">
          <div className="flex gap-4 relative h-full items-center">
            ...
            {/* Column 1: Outer Column (Moves Down/Opposite) */}
            <motion.div 
            ...
            {/* Column 2: Main Image Reel (Moves Up/Down) */}
            <div className="relative z-10 h-full flex items-center">
              <motion.div 
                className="flex flex-col gap-6"
                // This magic formula ensures the 'currentIndex' item is always in the visual center
            ...
            {/* Column 3: Outer Column (Moves Down/Opposite) */}
            <motion.div 
              className="flex flex-col gap-6"
              animate={{ y: (currentIndex * CARD_TOTAL_HEIGHT) - (TESTIMONIALS.length * CARD_TOTAL_HEIGHT / 2) }}
              transition={{ type: "spring", stiffness: 70, damping: 20 }}
            ...
            {/* Edge Vignette Blur to hide card cutoffs */}
        ...
        <div className="flex-1 p-20 flex flex-col justify-center bg-white">
          ...
          {/* Testimonial Text with Blur Animation */}
          <div className="flex-1 relative flex flex-col justify-center min-h-[200px]">
            <AnimatePresence mode="wait" custom={direction}>
              <motion.div
                ...
                className="w-full"
              >
                <h2 className="text-3xl md:text-4xl font-mono text-[#1a1a1a] leading-[1.35] mb-12 tracking-tight max-w-[540px] antialiased">
                  {testimonial.text}
                </h2>
                <div className="flex flex-col gap-1">
                  <p className="text-gray-400 font-medium text-lg font-mono tracking-tight">
                    {testimonial.author}
                  </p>
                </div>
              </motion.div>
          ...
          {/* Minimalist Controls & Counter */}
          <div className="mt-8 flex items-center justify-between max-w-[540px]">
